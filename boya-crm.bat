...
REM - backend / frontend 동시에 실행(각각 별도 창)
REM ==========================================

REM ---- 0) bat 파일이 있는 폴더로 이동 (UNC도 지원)
pushd "%~dp0"
set "ROOT=%CD%"
...
REM ---- 4) 프론트엔드 의존성 설치 (npm 확인)
where npm >nul 2>&1
if errorlevel 1 (
  echo [ERROR] npm(Node.js)가 설치되어 있지 않습니다. https://nodejs.org/ 에서 LTS 설치 후 재시도해주세요.
  popd
  pause
  exit /b 1
)
...
REM ---- 6) 백엔드 실행 (새 창, 시작 폴더 지정)
echo [INFO] 백엔드 실행 시작 (http://localhost:8080) ...
start "BOYA-CRM Backend" /D "%BACKEND%" cmd /k ""%VENV_PY%" app.py"

REM ---- 7) 프론트엔드 실행 (새 창, 시작 폴더 지정)
echo [INFO] 프론트엔드 실행 시작 (http://localhost:3000) ...
start "BOYA-CRM Frontend" /D "%FRONTEND%" cmd /k "npm run dev"
